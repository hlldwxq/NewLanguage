uint64 threshold=16
def uint64 q_adjacent_find(uint64* arr, uint64 first, uint64 last){
    if (first == last)
	then {return last}
...
def uint64 unique(uint64* arr, uint64 first, uint64 last){
    first = q_adjacent_find(arr,first,last)
